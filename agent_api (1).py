...
def exists(opp_id: str):
    """Check whether an opportunity has been processed (its validation.json is
    present in GCS). The UI calls this before starting so it can show a helpful
    alert instead of running the pipeline on a missing opportunity."""
    if AGENT_MOCK:
    ...
    try:
        ...
        found = vfile.get("status") == "success"
        return {
            "opportunity_id": opp_id,
            "exists": found,
            "status": vfile.get("status"),
            "path": vfile.get("path", ""),
        }
    except Exception as exc:  # noqa: BLE001
        # On an unexpected error, don't hard-block - report unknown so the UI
        # can decide; here we treat it as not-found with a message.
