...
def opportunity_exists(opportunity_id: str) -> dict:
    ...
    base = OPP_EXISTS_PREFIX.strip("/")
    prefix = f"{base}/{opp}/" if base else f"{opp}/"
    try:
        client = _client()
        bucket = client.bucket(OPP_EXISTS_BUCKET)
        # only need to know if at least one object exists under the folder
        blobs = client.list_blobs(bucket, prefix=prefix, max_results=1)
        found = any(True for _ in blobs)
        return {
            "opportunity_id": opp,
            "exists": found,
            "path": f"gs://{OPP_EXISTS_BUCKET}/{prefix}",
        }
    except Exception as exc:  # noqa: BLE001
        print(f"[exists] check failed gs://{OPP_EXISTS_BUCKET}/{prefix}: "
              f"{type(exc).__name__}: {exc}")
