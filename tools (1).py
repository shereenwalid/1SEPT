...
# ─────────────────────────────────────────────────────────────────────
def _app_log_blob_name(prefix: str, opp_id: str, session_id: str) -> str:
    """Deterministic log object name so /process and /feedback update the SAME
    file for a given run. Keyed by opp_id (both endpoints always know it), so
    no session id needs threading through the UI: <opp>.json"""
    safe_opp = str(opp_id or "unknown").strip().strip("/")
    name = f"{safe_opp}.json"
...
def save_app_log(session_id: str, opp_id: str, user: str = "unknown",
    ...
    One record per run (keyed by opp_id + session_id). Called at process time
    (feedback empty) and again when feedback is raised (feedback filled in),
    writing to the SAME object each time.
    """
    ...
    sess = str(session_id or opp_id or "nosession").strip()
    blob_name = _app_log_blob_name(prefix, opp_id, sess)

    try:
        bucket = _client().bucket(bucket_name)
        blob = bucket.blob(blob_name)

        # If a log already exists for this run, preserve its original datetime
        # and user, and merge feedback rather than overwrite blindly.
        existing = {}
        try:
            if blob.exists():
        ...
        record = {
            "session_id": sess,
            "datetime": existing.get("datetime") or (_dt.datetime.utcnow().isoformat() + "Z"),
        ...
        print(f"[app_log] save failed gs://{bucket_name}/{blob_name}: "
              f"{type(exc).__name__}: {exc}")
