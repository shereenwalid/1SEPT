...
# ─────────────────────────────────────────────────────────────────────
# validation.json  (agent no longer produces validation - it's a FILE)
# The pipeline that writes validation lands it at:
#     processed_files/<OPP-ID>/validation.json
# Shape:
#   { "validation": {status, reasoning, comments, date},
#     "opportunity_id": "...",
#     "report": { opportunity_id, matched_folder, order_category,
#                 required_information[], checks[], bsp_base_checks[] },
#     "feedbacks": [ {id, check_key, option, comment, by, date} ]  # UI-written
#   }
# ─────────────────────────────────────────────────────────────────────
VALIDATION_FILENAME = os.getenv("VALIDATION_FILENAME", "validation.json")
# The ingestion pipeline writes validation.json to:
#   gs://{VALIDATION_BUCKET}/{VALIDATION_PREFIX}{opp}/validation.json
# where (from the pipeline's config.yaml):
#   bucket = destination/input bucket   = "vfie-dh-customer-complex-fixed"
#   prefix = input_prefix + parsed_prefix
#          = "VFIE-Complex-Fixed-Products-VBOP-Extracts/" + "parsed_files/"
# Override via env if the pipeline config changes.
VALIDATION_BUCKET = os.getenv("VALIDATION_BUCKET", "vfie-dh-customer-complex-fixed")
VALIDATION_PREFIX = os.getenv(
    "VALIDATION_PREFIX",
    "VFIE-Complex-Fixed-Products-VBOP-Extracts/parsed_files",
)


def _validation_blob_name(opportunity_id: str) -> str:
    """{input_prefix}{parsed_prefix}<opp>/validation.json — matches the
    ingestion pipeline's _opp_dir + VALIDATION_FILENAME exactly."""
    opp = opportunity_id.strip().strip("/")
    base = VALIDATION_PREFIX.strip("/")
    return f"{base}/{opp}/{VALIDATION_FILENAME}" if base else f"{opp}/{VALIDATION_FILENAME}"


def _validation_bucket():
    return VALIDATION_BUCKET or GCS_BUCKET_NAME


def read_validation_file(opportunity_id: str) -> dict:
    """Read validation.json for an opportunity from GCS.

    Returns the parsed JSON (with report + validation + feedbacks), or a dict
    with status 'not_found' / 'error'.
    """
    blob_name = _validation_blob_name(opportunity_id)
    bkt = _validation_bucket()
    try:
        bucket = _client().bucket(bkt)
        blob = bucket.blob(blob_name)
        if not blob.exists():
            return {"status": "not_found", "opportunity_id": opportunity_id,
                    "message": f"No validation.json at gs://{bkt}/{blob_name}",
                    "path": f"gs://{bkt}/{blob_name}"}
        data = json.loads(blob.download_as_text())
        data.setdefault("feedbacks", [])
        data["status"] = "success"
        data["path"] = f"gs://{bkt}/{blob_name}"
        return data
    except Exception as exc:  # noqa: BLE001
        print(f"[validation] read failed gs://{bkt}/{blob_name}: "
              f"{type(exc).__name__}: {exc}")
        return {"status": "error", "opportunity_id": opportunity_id,
                "message": f"{type(exc).__name__}: {exc}",
                "path": f"gs://{GCS_BUCKET_NAME}/{blob_name}"}


def save_feedbacks(opportunity_id: str, feedbacks) -> dict:
    """Overwrite the 'feedbacks' array inside validation.json for an opportunity.

    Feedback is inserted ONLY from the UI (users raising requests). The agent's
    report/check rows are left untouched.

    Args:
        opportunity_id: The opportunity id.
        feedbacks: list of feedback dicts, each
            {id, check_key, option, comment, by, date}.
    """
    blob_name = _validation_blob_name(opportunity_id)
    bkt = _validation_bucket()
    try:
        bucket = _client().bucket(bkt)
        blob = bucket.blob(blob_name)
        # read current file (or start a minimal shell if missing)
        if blob.exists():
            data = json.loads(blob.download_as_text())
        else:
            data = {"opportunity_id": opportunity_id, "report": {}, "validation": {}}
        if isinstance(feedbacks, str):
            feedbacks = json.loads(feedbacks)
        data["feedbacks"] = feedbacks or []
        blob.upload_from_string(json.dumps(data, indent=2),
                                content_type="application/json")
        return {"status": "success",
                "path": f"gs://{bkt}/{blob_name}",
                "feedback_count": len(data["feedbacks"])}
    except Exception as exc:  # noqa: BLE001
        print(f"[validation] save_feedbacks failed gs://{bkt}/{blob_name}: "
              f"{type(exc).__name__}: {exc}")
        return {"status": "error", "opportunity_id": opportunity_id,
                "message": f"{type(exc).__name__}: {exc}"}
...
# The ingestion pipeline creates a folder named after the opportunity id
# under the processed_files prefix. If that folder exists, the opportunity
# has been processed and is ready; if not, it may still be processing.
#   gs://{OPP_EXISTS_BUCKET}/{OPP_EXISTS_PREFIX}/<OPP-ID>/
# ─────────────────────────────────────────────────────────────────────
OPP_EXISTS_BUCKET = os.getenv("OPP_EXISTS_BUCKET", "vfie-dh-customer-complex-fixed")
OPP_EXISTS_PREFIX = os.getenv(
    "OPP_EXISTS_PREFIX",
    "VFIE-Complex-Fixed-Products-VBOP-Extracts/processed_files",
)


def opportunity_exists(opportunity_id: str) -> dict:
    """Return whether a folder named <opportunity_id> exists under the
    processed_files prefix. Lists blobs with that prefix and returns True if
    any object is found (a folder 'exists' when it contains objects).

    Config is read inline (not module globals) so this never fails on a missing
    constant even if an older module copy is loaded."""
    bucket_name = os.getenv("OPP_EXISTS_BUCKET", "vfie-dh-customer-complex-fixed")
    prefix_base = os.getenv(
        "OPP_EXISTS_PREFIX",
        "VFIE-Complex-Fixed-Products-VBOP-Extracts/processed_files",
    ).strip("/")
    opp = str(opportunity_id).strip().strip("/")
    prefix = f"{prefix_base}/{opp}/" if prefix_base else f"{opp}/"
    try:
        client = _client()
        bucket = client.bucket(bucket_name)
        blobs = client.list_blobs(bucket, prefix=prefix, max_results=1)
        found = any(True for _ in blobs)
        return {
            "opportunity_id": opp,
            "exists": found,
        ...
        print(f"[exists] check failed gs://{bucket_name}/{prefix}: "
              f"{type(exc).__name__}: {exc}")
        return {"opportunity_id": opp, "exists": False,
...
# Each app run writes one JSON log to a logs folder in GCS:
#   gs://{APP_LOG_BUCKET}/{APP_LOG_PREFIX}/<opp_id>_<timestamp>.json
# Contents: session_id, datetime, user, opp_id, feedback[].
# Config is read inline so this never fails on a missing module constant.
# ─────────────────────────────────────────────────────────────────────
def _find_app_log_blob(client, bucket, prefix, session_id):
    """Find an existing log object for this session (name ends with
    _<session_id>.json). Returns the blob name if found, else None."""
    suffix = f"_{session_id}.json"
    search_prefix = f"{prefix}/" if prefix else ""
    try:
        for b in client.list_blobs(bucket, prefix=search_prefix):
...
def save_app_log(session_id: str, opp_id: str, user: str = "unknown",
                 feedback=None, append: bool = False) -> dict:
    """Write/update one run-log JSON object in the app-logs folder in GCS.

    The log is keyed by SESSION ID (one file per user session), so multiple
    users searching the same opportunity are tracked separately.

    File name: <date_time>_<session_id>.json  (date_time set once, at creation).

    Called first when the user clicks 'Begin Order Creation Process' (feedback
    empty), then again when a request is raised - it finds the same file by
    session id and merges the feedback in.
    """
    import datetime as _dt

    bucket_name = os.getenv("APP_LOG_BUCKET", "vfie-dh-customer-complex-fixed")
    prefix = os.getenv(
        "APP_LOG_PREFIX",
        "VFIE-Complex-Fixed-Products-VBOP-Extracts/quote-to-order-app-logs",
    ).strip("/")
    sess = str(session_id or "nosession").strip().strip("/")
    ...
    try:
        ...
        # Find an existing log for this session; if none, mint a new filename.
        existing_name = _find_app_log_blob(client, bucket, prefix, sess)
        if existing_name:
            blob_name = existing_name
        else:
            stamp = now.strftime("%Y-%m-%dT%H-%M-%S")
            fname = f"{stamp}_{sess}.json"
            blob_name = f"{prefix}/{fname}" if prefix else fname

        blob = bucket.blob(blob_name)

        # Preserve original datetime/user; merge feedback.
        existing = {}
        try:
            if blob.exists():
        ...
        # Decide the feedback list for this session's log.
        # - append=True: add the new request(s) to what THIS log already has
        #   (so the log only ever contains requests raised in THIS session,
        #   never other users' requests from the validation file).
        # - append=False: set the feedback to the given list (or keep existing).
        prior_fb = existing.get("feedback", []) or []
        if append and feedback:
            seen_ids = {f.get("id") for f in prior_fb if isinstance(f, dict)}
            merged = list(prior_fb)
            for f in feedback:
                if not isinstance(f, dict) or f.get("id") not in seen_ids:
                    merged.append(f)
                    seen_ids.add(f.get("id") if isinstance(f, dict) else None)
            fb_value = merged
        ...
        record = {
            "session_id": sess,
            "datetime": existing.get("datetime") or (now.isoformat() + "Z"),
            "user": existing.get("user") if existing.get("user") and existing.get("user") != "unknown" else (user or "unknown"),
            "opp_id": opp_id or existing.get("opp_id"),
            "feedback": fb_value,
        }

        blob.upload_from_string(json.dumps(record, indent=2),
                                content_type="application/json")
        print(f"[app_log] saved gs://{bucket_name}/{blob_name} "
              f"(feedback: {len(record['feedback'])})")
        return {"status": "success", "path": f"gs://{bucket_name}/{blob_name}",
                "record": record}
    except Exception as exc:  # noqa: BLE001
        print(f"[app_log] save failed (session {sess}): "
              f"{type(exc).__name__}: {exc}")
