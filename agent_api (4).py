...
async def process(opp_id: str, request: Request = None):
    ...
    # Write an app run log to GCS (best effort - never fails the request).
    if not AGENT_MOCK:
        try:
            from vf_quote_to_order_agent_configured.Agent.tools import (  # type: ignore
                save_app_log,
            ...
            save_app_log(
                ...
                opp_id=opp_id,
                user=_user,
                feedback=result.get("feedbacks", []),
            )
        except Exception as _log_exc:  # noqa: BLE001
            print(f"[app_log] could not write run log: {_log_exc}")
    ...
    try:
        ...
        if saved.get("status") == "success":
            ...
            try:
                ...
                save_app_log(session_id=opp_id, opp_id=opp_id, feedback=existing)
            except Exception as _log_exc:  # noqa: BLE001
